// Open the plugin UI
penpot.ui.open("GitHub Exporter", `${import.meta.env.BASE_URL}?theme=${penpot.theme}`, {
  width: 350,
  height: 580,
});
...
async function exportAssets() {
  try {
    ...
    for (const shape of nodes) {
      for (const exportConfig of shape.exports) {
        try {
          ...
          console.warn("[Plugin] Export failed for", shape.name);
    ...
    console.error("[Plugin] Export failed:", errorMessage);
    penpot.ui.sendMessage({ type: "no-assets" });
  }
}
